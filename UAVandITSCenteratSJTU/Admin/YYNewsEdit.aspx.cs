...
public partial class Admin_NewsEdit : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            try
            {
                ...
                this.DropDownList1.Text = newstype.ToString();
    ...
    protected void btnReturn_Click(object sender, EventArgs e)
    {
        try
        {
            string ss = "YYNewsReleaseManage.aspx" ;
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (ispicchanged)
            {
                sql = string.Format("update [News] set NewsTitle='{0}',NewsCont='{1}',NewsIsTop='{2}',NewsIsRed='{3}',NewsKeyWords='{4}',NewsPicUrl='{5}',NewsYY='{6}'  where ID={7}",
                    newstitle.ToString(), newscont.ToString(), newsistop.ToString(), newsisred.ToString(), newskeywords.ToString(), newspicrul.ToString(), this.DropDownList1.Text.ToString(),id.ToString());
            }
            else
            {
                sql = string.Format("update [News] set NewsTitle='{0}',NewsCont='{1}',NewsIsTop='{2}',NewsIsRed='{3}',NewsKeyWords='{4}',NewsYY='{5}'  where ID={6}",
                                  newstitle.ToString(), newscont.ToString(), newsistop.ToString(), newsisred.ToString(), newskeywords.ToString(),this.DropDownList1.Text.ToString(), id.ToString());
            }
            ...
            else
            {
                ...
                string ss = "<script language=javaScript>alert('修改成功！');window.location='YYNewsReleaseManage.aspx';</script>";
                Response.Write(ss);
            }
