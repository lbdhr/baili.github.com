...
public partial class NewsList : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            try
            {
                string newswriterid = Request.QueryString["navid"].ToString();
                string newswriter = SqlHelper.FindString("select SonNav from NavDetail where [ID]=" + newswriterid.ToString());
                InitData(newswriter.ToString());
            }
            catch
            { }
        }
    ...
    {
        DataSet DS = SqlHelper.ExecuteDataSet("SELECT * FROM [News] WHERE NewsWriter='"+type.ToString()+"' ORDER BY [NewsIsTop] ASC,[NewsIsRed] ASC,[ID] DESC ");
        table = DS.Tables[0];
